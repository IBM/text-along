import numpy as np
import os
from bias_tagger_detection.shared.data import REL2ID, POS2ID, TMI2ID
...
class Featurizer:
    ...
    def __init__(self, tok2id={}, pad_id=0, lexicon_feature_bits=1):
        ...
        self.rel2id = REL2ID
        self.tmi2id = TMI2ID
        if os.path.exists("bias_tagger_detection/lexicons/"):
            self.lexiconsdir = "bias_tagger_detection/lexicons/"
        elif os.path.exists(os.getcwd()+"/lexicons/"):
            self.lexiconsdir = os.getcwd()+"/lexicons/"
        else:
            raise FileNotFoundError("Lexicon file doesn't exists.")
        self.lexicons = {
            'assertives': self.read_lexicon(f'{self.lexiconsdir}assertives_hooper1975.txt'),
            'entailed_arg': self.read_lexicon(f'{self.lexiconsdir}entailed_arg_berant2012.txt'),
            'entailed': self.read_lexicon(f'{self.lexiconsdir}entailed_berant2012.txt'), 
            'entailing_arg': self.read_lexicon(f'{self.lexiconsdir}entailing_arg_berant2012.txt'), 
            'entailing': self.read_lexicon(f'{self.lexiconsdir}entailing_berant2012.txt'), 
            'factives': self.read_lexicon(f'{self.lexiconsdir}factives_hooper1975.txt'),
            'hedges': self.read_lexicon(f'{self.lexiconsdir}hedges_hyland2005.txt'),
            'implicatives': self.read_lexicon(f'{self.lexiconsdir}implicatives_karttunen1971.txt'),
            'negatives': self.read_lexicon(f'{self.lexiconsdir}negative_liu2005.txt'),
            'positives': self.read_lexicon(f'{self.lexiconsdir}positive_liu2005.txt'),
            'npov': self.read_lexicon(f'{self.lexiconsdir}npov_lexicon.txt'),
            'reports': self.read_lexicon(f'{self.lexiconsdir}report_verbs.txt'),
            'strong_subjectives': self.read_lexicon(f'{self.lexiconsdir}strong_subjectives_riloff2003.txt'),
            'weak_subjectives': self.read_lexicon(f'{self.lexiconsdir}weak_subjectives_riloff2003.txt')
        }
    ...
    def get_feature_names(self):
        ...
        tmi_names = list(list(zip(*sorted(self.tmi2id.items(), key=lambda x: x[1])))[0])

        return lexicon_feature_names + context_feature_names + pos_names + rel_names + tmi_names       
    ...
    def features(self, id_seq, rel_ids, pos_ids, tmi_ids):
        ...
        tmi_feats = np.zeros((len(tmi_ids), len(TMI2ID)))
        tmi_feats[range(len(tmi_ids)), tmi_ids] = 1
        
        feats = np.concatenate((feats, pos_feats, rel_feats, tmi_feats), axis=1)
    ...
    def featurize_batch(self, batch_ids, rel_ids, pos_ids, tmi_ids, padded_len=0):
        """ takes [batch, len] returns [batch, len, features] """

        batch_feats = [
            self.features(list(id_seq), list(rel_ids), list(pos_ids), list(tmi_ids)) 
            for id_seq, rel_ids, pos_ids, tmi_ids in zip(batch_ids, rel_ids, pos_ids, tmi_ids)]
