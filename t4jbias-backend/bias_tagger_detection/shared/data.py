...
TMI_TAGS = ['tmi', 'no tmi', '<UNK>']
TMI2ID = {x:i for i, x in enumerate(TMI_TAGS)}
...
def get_examples(data_path, tok2id, max_seq_len, 
    ...
    for i, (line) in enumerate(tqdm(open(data_path))):
        ...
        # if there is no tmi info
        if len(parts) == 11:
            # print("\nLEN of path is 11\n")
            # print(parts)
            [revid, pre, post, _, _, pos, rels, _, _, _, tmiclass] = parts
        # if there pos/rel/tmi info
        elif len(parts) == 7:
        ...
        # or len(tokens) != len(tmiclass) \  
        # print(".......Length of tokens.........",len(tokens), "\n")
        # print(".......lenght of tmiclass ", len(tmiclass), "\n")
        if len(tokens) != len(pre_tok_labels) \
            or len(tokens) != len(rels) \
            or len(tokens) != len(pos) \
            or len(tmiclass) != 1 or len(post_tokens) != len(post_tok_labels):
            skipped += 1
            continue
        ...
        try:
            ...
            tmi_ids = pad([TMI2ID.get(x, TMI2ID['<UNK>']) for x in tmiclass], 0)
        except KeyError:
...
def get_dataloader(data_path, tok2id, batch_size, 
    ...
    def collate(data):
        ...
        data = [
            ...
            rel_ids[:, :max_len], pos_ids[:, :max_len], categories, tmi_ids[:, :max_len]
        ]
        return data
    ...
    data = TensorDataset(
        ...
        #TODO: change this dtype to torch.bool
        ...
        torch.tensor(examples['tmi_ids'], dtype=torch.long))
    if ARGS.train != '':
